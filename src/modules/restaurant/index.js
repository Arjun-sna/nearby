...
const renderRestaurantData = (title, content) => (
  <div className={styles["detail-item-container"]}>
    ...
    <div className={styles["detail-content"]}>{content}</div>
  </div>
);
      ...
      <Row key={`${detailsLabels[i]}-${detailsLabels[i + 1]}`}>
        <Col md={6} xs={12}>
          {renderRestaurantData(
            detailsLabels[i],
            additionalDetails[detailsLabels[i]]
          )}
        </Col>
        <Col md={6} xs={12}>
          {renderRestaurantData(
            detailsLabels[i + 1],
            additionalDetails[detailsLabels[i + 1]]
          )}
        </Col>
      </Row>
    );
        ...
        <Row>
          <Col xs={8}>
            <div className={styles["header-details-section"]}>
              <div className={styles["name"]}>{restaurantData.name}</div>
          ...
          <Col xs={4}>
            <div
              ...
              style={{
                background: `#${
                  restaurantData.user_rating
                    ? restaurantData.user_rating.rating_color
                    : "red"
                }`
              }}
            >
